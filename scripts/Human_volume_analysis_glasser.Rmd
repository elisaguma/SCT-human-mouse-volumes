...
human_vols <- XXY_data_subcort_cort_filtered_unique[,11:ncol(XXY_data_subcort_cort_filtered_unique)] #Then subset the volumes for the control observations #vols from 54:461
XXY_demo <- XXY_data_subcort_cort_filtered_unique[,1:10]
...
allROI_XXY_Lm_wTBV_zscore= anatLm(~ DX_GROUP + AGE_cent + BrainSegVolNotVent.x,zscore_df_XXY_clean,zscore_df_XXY_clean[,11:ncol(zscore_df_XXY_clean)])
...
allROI_XXY_Lm_noTBV_zscore= anatLm(~ DX_GROUP + AGE_cent,zscore_df_XXY_clean,zscore_df_XXY_clean[,11:ncol(zscore_df_XXY_clean)])
...
human_vols_XYY <- XYY_data_subcort_cort_filtered_unique[,11:ncol(XYY_data_subcort_cort_filtered_unique)] #Then subset the volumes for the control observations; volumes 60:466
XYY_demo <- XYY_data_subcort_cort_filtered_unique[,1:10]
...
allROI_XYY_Lm_wTBV_zscore= anatLm(~ dx_group + AGE_cent + BrainSegVolNotVent.x,zscore_df_XYY_clean,zscore_df_XYY_clean[,11:ncol(zscore_df_XYY_clean)])
...
```{r}
# run liner model across all ROIs
allROI_XYY_Lm_noTBV_zscore= anatLm(~ dx_group + AGE_cent,zscore_df_XYY_clean,zscore_df_XYY_clean[,11:ncol(zscore_df_XYY_clean)])
